...
@rem /// Désactivation de quelques services ///
sc stop DiagTrack
...
REM /// Optimisation planificateur de tache ///
...
@rem /// Désactivation télémétrie et de la collecte de données ///
...
@REM Paramètres -> Confidentialité -> Géneral -> Autoriser les applications à utiliser mon identifiant publicitaire...
...
@REM WiFi Sense : Partage HotSpot : Désactiver
reg add "HKLM\Software\Microsoft\PolicyManager\default\WiFi\AllowWiFiHotSpotReporting" /v value /t REG_DWORD /d 0 /f
@REM WiFi Sense: Shared HotSpot Auto-Connect: Disable
reg add "HKLM\Software\Microsoft\PolicyManager\default\WiFi\AllowAutoConnectToWiFiSenseHotspots" /v value /t REG_DWORD /d 0 /f

@REM Remplacez les mises à jour Windows par "Notifier pour programmer le redémarrage"
reg add "HKLM\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings" /v UxOption /t REG_DWORD /d 1 /f
@REM Disable P2P Update downlods outside of local network
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\DeliveryOptimization\Config" /v DODownloadMode /t REG_DWORD /d 0 /f


REM /// Masquez le champ de recherche de la barre des tâches. Vous pouvez toujours rechercher en appuyant sur la touche Win et commencer à taper ce que vous recherchez ///
REM 0 = hide completely, 1 = show only icon, 2 = show long search box
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" /v "SearchboxTaskbarMode" /t REG_DWORD /d 0 /f

REM /// Désactiver les listes MRU (listes de raccourcis) des applications XAML dans le menu Démarrer ///
reg add "HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "Start_TrackDocs" /t REG_DWORD /d 0 /f

REM /// Configurer l'Explorateur Windows pour qu'il démarre sur ce PC au lieu de l'accès rapide ///
...
@rem Maintenant, quelques optimisations
REM /// Afficher les fichiers cachés dans l'explorateur ///
REM reg add "HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "Hidden" /t REG_DWORD /d 1 /f

REM /// Afficher les fichiers système super cachés dans l'explorateur ///
...
REM /// Afficher les extensions de fichiers dans l'explorateur ///
